...
#define CAM2MOTOR 0.1   //meter
#define BBX_JUMPTHRETHHOLD_X	60
#define BBX_JUMPTHRETHHOLD_Y	40
#define CAMHIGHT     400
#define CAMWIDTH 			600
//AngleF_Struct AUTOshootangle;
//攻击模式指令
...
*/
/**
  * @brief  MINIPC data process
  * @param  u8
  * @retval void
  */
unsigned char ucRxBuffer[250];
unsigned char ucRxCnt = 0;	
int isdetected,framecounter,RXstate=0;
																						 
void PCdataprocess(u8 ucData)
{
	int i,Sum;	
	
	if(RXstate==0&&ucData==0xFF)
	{
		RXstate=1;
		ucRxBuffer[0]=ucData;
	}
	else if(RXstate==1&&ucData==0xFE)
	{
		RXstate=2;
		ucRxBuffer[1]=ucData;
	}
		...
		else if(RXstate==9)
	{
		...
		framecounter++;
		ucRxBuffer[9]=ucData;
		EnemyDataBuffer[EnemyDataBufferPoint].X = (short)((ucRxBuffer[3]<<8)|ucRxBuffer[4]);
		EnemyDataBuffer[EnemyDataBufferPoint].Y = (short)((ucRxBuffer[5]<<8)|ucRxBuffer[6]);
		EnemyDataBuffer[EnemyDataBufferPoint].Z = (short)((ucRxBuffer[7]<<8)|ucRxBuffer[8]);
//		if(EnemyDataBufferPoint>10)
//		{
//				if((EnemyDataBuffer[EnemyDataBufferPoint].X-EnemyDataBuffer[EnemyDataBufferPoint-1].X)<BBX_JUMPTHRETHHOLD_X)
//				{
//					Enemy_now.X=EnemyDataBuffer[EnemyDataBufferPoint].X;
//				}
//				if((EnemyDataBuffer[EnemyDataBufferPoint].Y-EnemyDataBuffer[EnemyDataBufferPoint-1].Y)<BBX_JUMPTHRETHHOLD_Y)
//				{
//					Enemy_now.Y=EnemyDataBuffer[EnemyDataBufferPoint].Y;
//				}

//		}
		if(EnemyDataBuffer[EnemyDataBufferPoint].X>-250&&EnemyDataBuffer[EnemyDataBufferPoint].Y>-180)
		{
					Enemy_now.X=EnemyDataBuffer[EnemyDataBufferPoint].X;
					Enemy_now.Y=EnemyDataBuffer[EnemyDataBufferPoint].Y;
		}
...
//		if (ucRxBuffer[0]!=0xFF)
//		{
//			ucRxCnt=0;
//			return;

//		}
//		if (ucRxBuffer[1]!=0xFE)
//		{
//			ucRxCnt=0;
//			return;
//		}
//if(ucRxCnt<10)		return;
//else	{
//				isdetected=1;
...
//    ReturnData.H =  - atan(X / Z0+CAM2MOTOR) * 57.2958F;
		   ReturnData.H = -atan(X / (distance)+CAM2MOTOR) * 57.2958F;
    ...
    {
    //不考虑重力加速度
        ReturnData.V = -atan(Y0 / (distance)+CAM2MOTOR) * 57.2958F;
    }
    else
    {
    //考虑重力加速度
