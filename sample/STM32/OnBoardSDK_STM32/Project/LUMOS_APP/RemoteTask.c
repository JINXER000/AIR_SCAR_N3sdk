...
{
	return (fabs((float)ChassisSpeedRef.forward_back_ref)>=10 || fabs((float)ChassisSpeedRef.left_right_ref)>=10 || fabs((float)GimbalRef.yaw_speed_ref)>=10 || fabs((float)GimbalRef.pitch_speed_ref)>=10);
}
