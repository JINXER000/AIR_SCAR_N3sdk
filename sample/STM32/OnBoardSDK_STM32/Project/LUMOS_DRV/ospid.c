...
//extern volatile MPU6050_REAL_DATA   MPU6050_Real_Data;
 float pitchgoal,rollgoal, yawgoal,pitchnow,rollnow,yawnow;
...
{
	YawOPID.CurrentError=yawgoal-yawnow;
	YawOPID.Pout = YawOPID.P * YawOPID.CurrentError;
	
		if (YawOPID.CurrentError>10)
	{
		YawOPID.index=0;
	}
	else
	...
	YawOPID.Iout = YawOPID.Iout < -YawOPID.IMax ? -YawOPID.IMax : YawOPID.Iout;
		...
		YawOPID.Dout = YawOPID.D *(YawOPID.CurrentError-YawOPID.LastError);
	
	YawOPID.PIDout = YawOPID.Pout + YawOPID.index*YawOPID.Iout + YawOPID.Dout;
	YawOPID.PIDout = YawOPID.PIDout > YawOPID.PIDMax ? YawOPID.PIDMax : YawOPID.PIDout;
	YawOPID.PIDout = YawOPID.PIDout < -YawOPID.PIDMax ? -YawOPID.PIDMax : YawOPID.PIDout;
	
		YawOPID.LastError = YawOPID.CurrentError;
		
if(fabs(YawOPID.CurrentError)<YawOPID.deadbond)	YawOPID.PIDout=0;
	
#if defined SINGLELOOP

	return (short)YawOPID.PIDout;
#elif defined DUALLOOP
	/***************************************	内环	******************************************/
